...
$apiEndPoints['export-ranges'] = '/api/import_network_ranges/';

$sidusisSelectedDivision = ConfigHelper::getConfig('sidusis.selected_division');
$sidusisTemporaryFilepath = sys_get_temp_dir() . DIRECTORY_SEPARATOR . 'lms-sidusis-' . time() . '.zip';
$sidusisOperatorOfferUrl = ConfigHelper::getConfig('sidusis.operator_offer_url', 'http://firma.pl/offer/');

$did = in_array('division', $options) ? intval($options['division']) : $sidusisSelectedDivision;

function getSidusisRangesReport($divisionid) {
    global $LMS, $DB, $SIDUSIS_LINKTECHNOLOGIES, $sidusisOperatorOfferUrl;
...
if (array_key_exists('export-ranges', $options)) {
	...
	CURLOPT_VERBOSE => array_key_exists('debug', $options) ? true : false,
    );
    $ch = curl_init($apiHost . $apiEndPoints['export-ranges']);
    curl_setopt_array($ch, $curl_options);
    $response = curl_exec($ch);
    if (array_key_exists('debug', $options)) { print_r(json_decode($response)); }
    curl_close($ch);
